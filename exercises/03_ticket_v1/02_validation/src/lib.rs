...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        ...
        if title.len() > 50 { panic!("Title cannot be longer than 50 characters"); }
        if description.is_empty() { panic!("Description cannot be empty"); }
        if description.len() > 500 { panic!("Description cannot be longer than 500 characters"); }
        if !matches!(status.as_str(), "To-Do" | "In Progress" | "Done") { panic!("Only `To-Do`, `In Progress`, and `Done` statuses are allowed"); }
        Self {
            title,
            description,
            status,

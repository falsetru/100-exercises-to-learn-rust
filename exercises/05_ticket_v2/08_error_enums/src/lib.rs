...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(TicketNewError::DescError(_)) => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescError("Description cannot be longer than 500 characters".to_string()));
        }
